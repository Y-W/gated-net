    ...
    with slim.arg_scope(resnet_arg_scope()):
      ...
      with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
        with tf.variable_scope('CondBranchFn', values=[net]):
          ...
          branch_val = resnet_v2_cifar10_ending_block(net, tmp_end_points, num_branches * 2)
          end_points['branch_fn'] = tmp_end_points
          softmax_input, noise_input = tf.split(branch_val, 2, axis=1)
          end_points['branch_preact'] = softmax_input
          end_points['branch_noise'] = noise_input
          branch_val = stochastic_branch_fn(softmax_input, noise_input, is_training)
